module.exports = class MartianRover {
  constructor() {
    this.data = {
      previousPosition: {
        x: 0,
  ...
  get move() {
    for (let instruction of this.instructions) {
      ...
      switch (instruction) {
        case "r":
          this.finalPosition.orientation = this.checkWheel(
            "r",
  ...
  checkWheel(instruction, orientation) {
    const wheel = ["n", "e", "s", "w"];

    let index;
    if (instruction === "r") {
      index = wheel.indexOf(orientation) + 1;
    }
    ...
    return this.newBearing(index, wheel);
  }

  newBearing(index, wheel) {
    if (index > 3) {
      return wheel[0];
    }
  ...
  isOnLostCoordinate() {
    ...
    const isOnLostCoordinate =
      stringLostCoordinates.includes(stringFinalPosition);
